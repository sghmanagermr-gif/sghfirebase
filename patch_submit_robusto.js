...
const targetRegex = /\/\/ Desplegar Formulario Personal SOLO si se ingresó alguna matrícula\s*if \(typeof window\.mostrarFormularioPersonal === "function" && Object\.keys\(matricula\)\.length > 0\) \{\s*window\.mostrarFormularioPersonal\(\);\s*\}/g;

if (targetRegex.test(content)) {
    content = content.replace(targetRegex, replaceStr);
    fs.writeFileSync(pathMain, content, 'utf8');
    console.log('Archivo main.js actualizado exitosamente en el submit.');
} else {
    console.log('No se encontro la cadena objetivo en main.js en el submit.');
}
