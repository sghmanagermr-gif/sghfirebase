...
var CONFIG_GLOBAL = {
  ...
  api_global: {
    url: "https://script.google.com/macros/s/AKfycbxpcgKB4HTQXRxC3aALwdEtZzg4VWyJryDGjBy4WRUkQnj9RIKR0EALhZLdPkPuk9LLzw/exec",
    url_invalidar: "https://script.google.com/macros/s/AKfycbxpcgKB4HTQXRxC3aALwdEtZzg4VWyJryDGjBy4WRUkQnj9RIKR0EALhZLdPkPuk9LLzw/exec?accion=invalidar"
  }
};
...
function obtenerParametrosApp() {
  try {
    ...
    try {
      ...
      // Falla silenciosa: adminUrl queda vacía, el sistema continúa operando
      console.log("[SGH] No se pudo leer adminUrl de bd_sgh.json: " + e.message);
    }
...
function validarAccesoUsuario(credenciales) {
  ...
  try {
    ...
    // --- Paso 6: Intentar recuperar datos guardados previamente de la hoja PLANTEL ---
    var matriculaGuardada = null;
    var vacantesGuardadas = null;
    try {
      var ssPlantel = SpreadsheetApp.openById(busqueda.idHojaPlantel);
      var hojaPlantel = ssPlantel.getSheetByName("PLANTEL");
      if (hojaPlantel) {
        var fila2 = hojaPlantel.getRange(2, 43, 1, 33).getValues()[0]; // AQ (43) a BW (75)
        
        var valMg = fila2[28] || "";
        var valMt = fila2[29] || "";
        var totalSecMg = 0;
        var totalSecMt = 0;
        var seccionesPorAnioObj = {};

        if (typeof valMg === 'string' && valMg.indexOf('{') === 0) {
          try {
            var objMg = JSON.parse(valMg);
            for (var plan in objMg) {
              seccionesPorAnioObj[plan] = objMg[plan];
              for (var anio in objMg[plan]) totalSecMg += parseInt(objMg[plan][anio]) || 0;
            }
          } catch(e) {}
        } else {
          totalSecMg = parseInt(valMg) || 0;
        }

        if (typeof valMt === 'string' && valMt.indexOf('{') === 0) {
          try {
            var objMt = JSON.parse(valMt);
            for (var plan in objMt) {
              seccionesPorAnioObj[plan] = objMt[plan];
              for (var anio in objMt[plan]) totalSecMt += parseInt(objMt[plan][anio]) || 0;
            }
          } catch(e) {}
        } else {
          totalSecMt = parseInt(valMt) || 0;
        }

        matriculaGuardada = {
          matFem: fila2[0] || 0, matMas: fila2[1] || 0, matTot: fila2[2] || 0,
          preFem: fila2[3] || 0, preMas: fila2[4] || 0, preTot: fila2[5] || 0,
          iniFem: fila2[6] || 0, iniMas: fila2[7] || 0, iniTot: fila2[8] || 0,
          priFem: fila2[9] || 0, priMas: fila2[10] || 0, priTot: fila2[11] || 0,
          mgFem: fila2[12] || 0, mgMas: fila2[13] || 0, mgTot: fila2[14] || 0,
          mtFem: fila2[15] || 0, mtMas: fila2[16] || 0, mtTot: fila2[17] || 0,
          aduFem: fila2[18] || 0, aduMas: fila2[19] || 0, aduTot: fila2[20] || 0,
          espFem: fila2[21] || 0, espMas: fila2[22] || 0, espTot: fila2[23] || 0,
          granTot: fila2[24] || 0,
          secMat: fila2[25] || 0, secPre: fila2[26] || 0, secPri: fila2[27] || 0,
          secMg: totalSecMg, secMt: totalSecMt, secAdu: fila2[30] || 0, secEsp: fila2[31] || 0,
          seccionesPorAnio: seccionesPorAnioObj
        };

        vacantesGuardadas = fila2[32] || ""; // BW (75)
      }
    } catch(e) {
      console.log("[SGH] No se pudo precargar matricula/vacantes previas: " + e.message);
    }
    ...
    return {
      ...
      matriculaPrevia: matriculaGuardada,
      vacantesPrevia: vacantesGuardadas
    };
...
function _construirFilaRegistro(payload, usuario, filaDestino) {
  ...
  return [[
    ...
    String(payload.observaciones || ''),           // BX - OBSERVACION
...
function guardarDatosPlantel(payload) {
  try {
    ...
    var jsonMg = {};
    var jsonMt = {};
    for (var plan in secAnio) {
      if (plan.startsWith('3')) jsonMg[plan] = secAnio[plan];
      if (plan.startsWith('4')) jsonMt[plan] = secAnio[plan];
    }
    var valMg = Object.keys(jsonMg).length > 0 ? JSON.stringify(jsonMg) : (m.secMg || "");
    var valMt = Object.keys(jsonMt).length > 0 ? JSON.stringify(jsonMt) : (m.secMt || "");

    // Mapeo exacto de las 75 columnas (A-BW)
    var filaData = [
      ...
      // SECCIONES
      m.secMat, m.secPre, m.secPri, valMg, valMt, m.secAdu, m.secEsp // BP-BV
    ];
...
function guardarRegistroPersonal(payload) {
  try {
      ...
      for (var i = 0; i < cedulas.length; i++) {
        ...
        if (cedFila === nuevaCedula) {
          if (!esEdicion || (esEdicion && idEdicion !== nuevaCedula)) {
            return { 
              exito: false, 
              esDuplicadoEstatal: true, 
              error: "El número de Cédula " + nuevaCedula + " se encuentra duplicada. Verifique o comuníquese con el responsable de Gestión Humana de su municipio." 
            };
          }
        }
    ...
    // Variables necesarias para validación global
    var nuevaCedula = String(payload.cedula).trim();
    var esEdicion = payload.idEdicion ? true : false;
    var idEdicion = esEdicion ? String(payload.idEdicion).trim() : "";

    // === VALIDACIÓN GLOBAL (A NIVEL ESTATAL) ===
    if (!esEdicion || (esEdicion && idEdicion !== nuevaCedula)) {
      try {
        var depActual = payload.dependenciaLaboral ? String(payload.dependenciaLaboral).trim().toUpperCase() : "";
        var urlApi = CONFIG_GLOBAL.api_global.url + "?cedula=" + encodeURIComponent(nuevaCedula) + "&dependencia=" + encodeURIComponent(depActual);
        var respuestaApi = UrlFetchApp.fetch(urlApi);
        var datosApi = JSON.parse(respuestaApi.getContentText());
        
        if (datosApi.exito && datosApi.existe) {
          return { 
            exito: false, 
            esDuplicadoEstatal: true,
            error: "El número de Cédula " + nuevaCedula + " se encuentra duplicada. Verifique o comuníquese con el responsable de Gestión Humana de su municipio." 
        ...
        return { exito: false, error: "Error al validar la cédula en la Base de Datos Estatal: " + e.message };
      }
    }
    ...
    var filaData = [
      ...
      _U(payload.observaciones),                      // BW: OBSERVACIÓN
    ...
    var esMediaAtiende = (payload.atiendeMatricula === "SI" && (
      ...
      payload.nivelModalidad === "MEDIA GENERAL - MEDIA TECNICA" ||
    ...
    // Invalidar caché de la API Global para reflejar el cambio en la próxima consulta (Regla de Oro 4)
    try {
      UrlFetchApp.fetch(CONFIG_GLOBAL.api_global.url_invalidar, { muteHttpExceptions: true });
    } catch(eCacheInv) {
      console.warn("[SGH] No se pudo invalidar la caché de la API Global: " + eCacheInv.message);
    }
...
function eliminarRegistroPersonal(payload) {
  try {
    ...
    // Invalidar caché de la API Global para reflejar la baja en la próxima consulta (Regla de Oro 4)
    try {
      UrlFetchApp.fetch(CONFIG_GLOBAL.api_global.url_invalidar, { muteHttpExceptions: true });
    } catch(eCacheInvE) {
      console.warn("[SGH] No se pudo invalidar la caché de la API Global tras eliminación: " + eCacheInvE.message);
    }
